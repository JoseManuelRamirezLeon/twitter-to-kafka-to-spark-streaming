    ...
    def run(self):
        ...
        while True:
            for message in consumer:
                ...
                inicio_texto = ',"text":'
                fin_texto = ',"source":'
                tweet_string = tweet_string[tweet_string.find(inicio_texto)+8:]
                tweet_string = tweet_string[:tweet_string.find(fin_texto)]
                print(tweet_string)
                kafkaParams = {"bootstrap.servers": "localhost:9092"}
                stream = KafkaUtils.createDirectStream(ssc, ['twitter'], kafkaParams)
